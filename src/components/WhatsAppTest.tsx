...
export function WhatsAppTest({ company }: { company: string }) {
  ...
  async function verify() {
    ...
    try {
      ...
      setVerification(data as WhatsAppVerification);
    } catch (err) {
  ...
  const waLink =
    verification?.found && verification.number
      ? `https://wa.me/${verification.number}?text=${encodeURIComponent(WHATSAPP_TEST_QUESTION)}`
      : null;
  ...
  return (
    <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5">
      <div className="mb-3 flex items-center gap-2">
        ...
        <h4 className="text-base font-semibold">Atendimento por WhatsApp</h4>
      </div>

      {!verification && !verifying && (
        <>
          <p className="mb-3 text-sm opacity-80">
            Vou carregar o site oficial da empresa e procurar links de WhatsApp (wa.me, api.whatsapp.com) no HTML.
          </p>
          <button
            type="button"
            onClick={verify}
      ...
      {verification && (
        <div className="space-y-3">
          <div className="flex items-start gap-3 rounded-lg bg-white/60 p-3 text-sm dark:bg-white/5">
            {verification.found ? (
            ...
            <div className="min-w-0 flex-1">
              {verification.found ? (
                <>
                  <div className="font-semibold">WhatsApp encontrado: {verification.display}</div>
                  {verification.source && (
                    <div className="text-xs opacity-70">{verification.source}</div>
                  )}
                </>
              ) : (
          ...
          {verification.found && waLink && (
            <div className="space-y-2">
              ...
              <button
                ...
                className="ml-2 inline-flex items-center gap-1 rounded-lg border border-black/15 px-3 py-1.5 text-sm transition hover:bg-black/5 dark:border-white/15 dark:hover:bg-white/10"
              >
      ...
      {verification?.found && showForm && (
        <form onSubmit={submitTest} className="mt-4 space-y-3 rounded-lg border border-black/10 bg-white/60 p-3 text-sm dark:border-white/10 dark:bg-white/5">
          ...
          >
            ...
            {analyzing ? "Analisando…" : "Analisar resposta"}
          </button>
        </form>
      )}
      ...
      {analysis && (
        <div className="mt-4 space-y-3 rounded-lg border border-indigo-500/30 bg-indigo-500/5 p-4 text-sm">
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            <div>
              <div className="text-xs font-semibold uppercase tracking-wide opacity-70">Classificação</div>
